...
---- Insert process record with status 'Not ADAPTED'    ----------------------------------------------------------------------------------------------------------------------------------
  function insert_change_adaptation_process(in_scenario_id in changeadaptationscenario.cas_id%type,
...
---- Insert manual condition fulfillment records --------------------------------------------------------------------------------------------------------------------------------------------
  procedure insert_manual_condition_fulfillment(in_process_id  in changeadaptationprocess.cap_id%type,
  ...
  begin
    for condition in (select distinct c.cacm_condition_id as condition_id
      ...
      v_manualconditionfulfillment.camcf_id := ca_manualconditionfulfillment_SQ.nextval;
  ...
  end insert_manual_condition_fulfillment;
    ...
    for new_change in (select *
      ...
      if v_change_type is not null then
          ...
          for scenario_step in (select cas.cas_id
            ...
            v_process_id := insert_change_adaptation_process(scenario_step.cas_id, new_change.ch_id);
            insert_manual_condition_fulfillment(v_process_id, scenario_step.cas_id, new_change.ch_id);
...
---- Counts how many manual conditions of the process are not fulfilled yet -----------------------------------------------------------------------------------------------------------------
  function manual_not_fulfilled_condition_count(in_process_id in changeadaptationprocess.cap_id%type) return number as
  ...
  end manual_not_fulfilled_condition_count;
...
---- Check all conditions--------------------------------------------------------------------------------------------------------------------------------------------------------------------
  ...
  begin
    ...
    if v_coditions_fulfilled and manual_not_fulfilled_condition_count(in_process_id) > 0 then
      v_coditions_fulfilled := false;
    end if;
...
---- Add dataset to data highway level ------------------------------------------------------------------------------------------------------------------------------------------------------
  procedure add_dataset_to_datahighwaylevel(in_change_id in change.ch_id%type) is
  ...
  end add_dataset_to_datahighwaylevel;
    
---- Add dataset to the first data highway level
  procedure add_dataset_to_1st_datahighwaylevel(in_change_id in change.ch_id%type) is
    v_dataset_id dataset.ds_id%type;
  begin
    v_dataset_id := get_additional_dataset_id(in_change_id);
    --sets data highway level of the data set   
    update dataset
     ...
     where ds_id = v_dataset_id;
  end add_dataset_to_1st_datahighwaylevel;
